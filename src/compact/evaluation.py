...
def ari_ami(left, right, report_type = 'ARI'):
    ...
    if report_type == 'ARI':
        ...
        raise ValueError(f"type {report_type} not supported. only 'ARI' or 'AMI' allowed")
    return result
...
def mmr(predicted, reference, report_best_matches = False):
    ...
    for ref_name,ref_set in reference.items():
        overlap_scores = {name:overlap(ref_set,pred_set) for name,pred_set in predicted.items()}
...
def get_count_table(predicted, reference):
    ...
    used to calculate sensitivity and ppv
    """
    count_series = {}
    for ref_name,ref_set in reference.items():
        count_dict = {}
...
def sensitivity(predicted, reference, disregard_unclustered = True):
    ...
    Args:
        predicted, reference: dicts
        disregard_unclustered(bool):wheter to include 'unclustered' 
        as cluster set in Sn computation. if True, still uses unclustered
